...
class DefaultSlotbookApiClient extends SlotbookApiClient {
  ...
  val apiUrl = "http://128.199.32.146/api"
   ...
   * @return
   */
  override def listCompaniesByService(
    serviceId: Service.ID,
    location: Option[LatLng],
    searchRadius: Option[Int] = Some(defaultSearchDistance))(implicit lang: Lang): Future[Seq[CompanyDistanceRating]] = {
      ...
      wsClient
        ...
        .post(
          new Event(
            ...
            endTime = slotTimes._2).toJson).map { response =>
            println(response)
          }
    }
